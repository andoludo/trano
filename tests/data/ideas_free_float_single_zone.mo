...
extends Modelica.Icons.MaterialPropertiesPackage;
      ...
      final checkLowPerformanceGlazing=false,
          mats={ideas_free_float_single_zone.Data.Materials.id_100(d=0.003),ideas_free_float_single_zone.Data.Materials.Air(d=0.0127),ideas_free_float_single_zone.Data.Materials.id_100(d=0.003)    },
...
package Constructions "Library of building envelope constructions"record external_wall
   ...
   extends IDEAS.Buildings.Data.Interfaces.Construction(
      mats={ideas_free_float_single_zone.Data.Materials.concrete(d=0.2),ideas_free_float_single_zone.Data.Materials.insulation_board(d=0.02),ideas_free_float_single_zone.Data.Materials.plywood(d=0.1)    });
    end external_wall;
end Constructions;
end Data;
...
"Medium in the component"
  ...
  inner IDEAS.BoundaryConditions.SimInfoManager sim(interZonalAirFlowType=IDEAS.BoundaryConditions.Types.InterZonalAirFlow.OnePort)
    ...
    redeclare parameter ideas_free_float_single_zone.Data.Constructions.external_wall constructionType,
    A={ 10, 10, 10, 10 },
    ...
    Placement(transformation(origin = { 197.73260621839557, -126.60172154793622 }, extent = {{-10, -10}, {10, 10}}
)));    IDEAS.Buildings.Components.Window[1] merged_win1_1(
    redeclare parameter ideas_free_float_single_zone.Data.Glazing.double_glazing glazing,
    A={ 1 },
    final azi={ 45 },
    final inc={ IDEAS.Types.Tilt.Wall })  annotation(
    Placement(transformation(origin = { -196.0945373156668, 106.73132964634135 }, extent = {{-10, -10}, {10, 10}}
)));IDEAS.Buildings.Components.SlabOnGround floor_2(
    redeclare parameter ideas_free_float_single_zone.Data.Constructions.external_wall constructionType,
    A=10)  annotation(
    Placement(transformation(origin = { -192.7132550728217, -121.57779951271387 }, extent = {{-10, -10}, {10, 10}}
)));
equation    connect(space_1.propsBus[1:4],merged_w1_1_w2_1_w3_1_w4_1[1:4].propsBus_a)
annotation (Line(
points={{ 0.0, 50.0 }    ,{ 98.86630310919779, 50.0 }    ,{ 98.86630310919779, -126.60172154793622 }    ,{ 197.73260621839557, -126.60172154793622 }    },
color={255,204,51},
thickness=0.5,
smooth=Smooth.None));    connect(space_1.propsBus[5],merged_win1_1[1].propsBus_a)
annotation (Line(
points={{ 0.0, 50.0 }    ,{ -98.0472686578334, 50.0 }    ,{ -98.0472686578334, 106.73132964634135 }    ,{ -196.0945373156668, 106.73132964634135 }    },
color={255,204,51},
thickness=0.5,
smooth=Smooth.None));    connect(space_1.propsBus[6],floor_2.propsBus_a)
annotation (Line(
points={{ 0.0, 50.0 }    ,{ -96.35662753641085, 50.0 }    ,{ -96.35662753641085, -121.57779951271387 }    ,{ -192.7132550728217, -121.57779951271387 }    },
color={255,204,51},
thickness=0.5,
smooth=Smooth.None));
