...
annotation (Line(points={{10,0},{110,0}}, color={0,0,127}));  connect(u, conVal.u_m) annotation (Line(points={{-118,0},{-22,0},{-22,-20},{0,
        ...
        model BoilerControlBoilercontrol_0
...
coordinateSystem(preserveAspectRatio=false)));connect(dataBus.yPumBoiPump_0, mulMax.u[1]);
connect(dataBus.yBoiler_0, booToRea.y);
end BoilerControlBoilercontrol_0;
        model DataServer
replaceable package Medium = Modelica.Media.Interfaces.PartialMedium;
...
connect(sum1.y, loa.Pow) annotation (Line(points={{-51,-26},{-38,-26},{-38,-76},
    {24,-76},{24,-49},{16,-49}}, color={0,0,127}));
            ...
            model BoilerWithoutStorageBoiler_0
    extends house_model.Trano.Fluid.Boilers.PartialBoilerWithoutStorage;
    Trano.Controls.BaseClasses.DataBus dataBus
    annotation (Placement(transformation(
  extent={{-120,-18},{-80,22}}), iconTransformation(extent={{-120,62},{-78,98}})));
    equation
    connect(dataBus.yBoiler_0, pumpDemandSignal.u);
connect(dataBus.gas_usageBoiler_0, GasUsage.y);
connect(dataBus.supply_temperatureBoiler_0, TSupply.T);
connect(dataBus.return_temperatureBoiler_0, TReturn.T);
connect(dataBus.m_flowBoiler_0, senVolFlo.V_flow);
     end BoilerWithoutStorageBoiler_0;
